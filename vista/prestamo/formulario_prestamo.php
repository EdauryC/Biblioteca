	...
	<body>
		...
		?>
				<nav class="navbar navbar-expand-lg  navbar-dark bg-company-red px-4">
					...
					<a class="navbar-brand mx-auto" href="../../vista/prestamo/formulario_prestamo.php">Lista de Prestamos Realizados</a>
					<a class="nav-item nav-link text-white" href="../../vista/prestamo/listar_prestamo.php">Regresar a la lista</a>
				</nav>
					...
					<div class="table-responsive">
						...
						<table class="table table-responsive mx-auto">
							...
							while (($fila = $registros->fetch_assoc()) > 0) {
								...
								echo "</tr>";
								$suma = $fila['id_prestamo'] . ',' . $fila['status'] . ',' . $fila['fecha_prestamo'] . ',' . $fila['fecha_vencimiento'] . ',' . $fila['usuario_id_usuario'] . ',' . $fila['empleado_id_empleado'] . '|';
